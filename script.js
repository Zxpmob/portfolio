...
async function initChat(user) {
  ...
  if (!user || !sbClient) {
    ...
    chatOpenEl.style.display = 'none';
    return;
  }

  chatLocked.style.display = 'none';
  chatOpenEl.style.display = 'flex';

  let { data: convo } = await sbClient
    .from('conversations').select('*').eq('customer_id', user.id).maybeSingle();

  if (!convo) {
    const meta = user.user_metadata || {};
    const fullName = [meta.first_name, meta.last_name].filter(Boolean).join(' ') || user.email;
    const { data: newConvo, error } = await sbClient
      .from('conversations').insert({ customer_id: user.id, customer_name: fullName })
      .select().single();
    if (error) { console.error('Could not start conversation:', error.message); return; }
    convo = newConvo;
  }
