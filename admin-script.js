...
if (!isConfigured) {
  ...
  async function loadStats() {
    ...
    const { count: totalViews, error: err1 } = await sb
      .from('page_views').select('*', { count: 'exact', head: true });

    const since24h = new Date(Date.now() - 24 * 60 * 60 * 1000).toISOString();
    const { count: views24h } = await sb
      .from('page_views').select('*', { count: 'exact', head: true }).gte('created_at', since24h);

    const { count: totalMessages } = await sb
      .from('messages').select('*', { count: 'exact', head: true });

    if (err1) {
      gridEl.innerHTML = `<div class="empty-state">${currentLang === 'fa' ? 'خطا در دریافت آمار' : 'Error loading stats'}: ${err1.message}</div>`;
      return;
    }
    ...
    gridEl.innerHTML = `
      <div class="stat-box"><span class="num">${totalViews ?? 0}</span><span class="label">${labels[0]}</span></div>
      <div class="stat-box"><span class="num">${views24h ?? 0}</span><span class="label">${labels[1]}</span></div>
      <div class="stat-box"><span class="num">${totalMessages ?? 0}</span><span class="label">${labels[2]}</span></div>
    `;
  }
  ...
  async function loadConversations() {
    ...
    listEl.innerHTML = data.map(c => `
      <button class="convo-item" data-id="${c.id}" data-name="${escapeHtml(c.customer_name || 'User')}">${escapeHtml(c.customer_name || 'User')}</button>
    `).join('');

    listEl.querySelectorAll('.convo-item').forEach(btn => {
      btn.addEventListener('click', () => openConversation(btn.dataset.id, btn.dataset.name));
    });
  }
  ...
  async function openConversation(id, name) {
    ...
    threadEl.innerHTML = `
      <div class="chat-thread-header">${escapeHtml(name)}</div>
